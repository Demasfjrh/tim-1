const cars = [
  ...
  },
  {
    ...
    image: 'https://via.placeholder.com/300x200?text=Daihatsu+Xenia',
  },
  {
    id: 5,
    name: 'Ford Mustang',
  ...
  },
  {
    ...
    image: 'https://via.placeholder.com/300x200?text=Kia+Carnival',
  },
];
...
function buyCar(id) {
  ...
  alert(
    `Anda telah membeli ${
      car.name
    } seharga Rp ${car.price.toLocaleString()}.`
  );
...
function editDescription(id) {
  ...
  // Prompt pengguna untuk memasukkan deskripsi baru
  const newDescription = prompt(
    'Masukkan deskripsi baru:',
    car.description
  );
  if (newDescription !== null && newDescription.trim() !== '') {
    // Perbarui deskripsi dalam database
    car.description = newDescription;

    // Perbarui deskripsi di DOM
...
function addCar(event) {
  ...
  // Tambahkan mobil baru ke array
  cars.push({
    id: cars.length + 1, // ID otomatis
    name,
    price,
    kategori: category,
    description,
